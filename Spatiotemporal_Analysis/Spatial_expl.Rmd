...
The Figure 2 is composed of 3 following plots that were assembled with Inkscape.

### Distribution range of unknown Syndiniales clusters of each Order

```{r}
#~# Figure 2: main plot
# Create data-frame
shared_loc <- c(rep("N=6",5), rep("N=5",5), rep("N=4",5), rep("N=3",5), rep("N=2",5), rep("N=1",5))
tax_status <- rep(c("Dino-Group-I","Dino-Group-II","Dino-Group-III","Dino-Group-V", "Unknown"), 6)
CC_nb <- c(nrow(df_I_6),nrow(df_II_6),nrow(df_III_6),nrow(df_V_6),nrow(df_uk_6),
           nrow(df_I_5),nrow(df_II_5),nrow(df_III_5),nrow(df_V_5),nrow(df_uk_5),
           nrow(df_I_4),nrow(df_II_4),nrow(df_III_4),nrow(df_V_4),nrow(df_uk_4),
           nrow(df_I_3),nrow(df_II_3),nrow(df_III_3),nrow(df_V_3),nrow(df_uk_3),
           nrow(df_I_2),nrow(df_II_2),nrow(df_III_2),nrow(df_V_2),nrow(df_uk_2),
           nrow(df_I_1),nrow(df_II_1),nrow(df_III_1),nrow(df_V_1),nrow(df_uk_1))
CC_perc <- c(UK_6_I,UK_6_II,UK_6_III,UK_6_V,UK_6_uk,
             UK_5_I,UK_5_II,UK_5_III,UK_5_V,UK_5_uk,
             UK_4_I,UK_4_II,UK_4_III,UK_4_V,UK_4_uk,
             UK_3_I,UK_3_II,UK_3_III,UK_3_V,UK_3_uk,
             UK_2_I,UK_2_II,UK_2_III,UK_2_V,UK_2_uk,
             UK_1_I,UK_1_II,UK_1_III,UK_1_V,UK_1_uk)
df <- data.frame(shared_loc,tax_status,CC_perc,CC_nb)

# Plot
ggplot(df, aes(fill=shared_loc, y=CC_perc, x=tax_status)) + 
  geom_bar(stat="identity", colour="black", fill="gray27") +
  theme_linedraw() +
  facet_grid(~shared_loc, scales = "free_x") +
...
### Occurence patterns of unknown Syndiniales clusters of each Order

Occurence pattern for endemic clusters (N=1)

```{r}
# Create data-frame
d1 <- data[data$N==1,]
d1_agr <- aggregate(d1$Location1, list(d1$Location1, d1$Order), FUN = length) # stats package
names(d1_agr)[1] <- "Location1"
names(d1_agr)[2] <- "Order"
names(d1_agr)[3] <- "Value"

# Calculate number of endemic CCs
sum(d1_agr$Value)

d1_agr$Location1 <- factor(d1_agr$Location1, levels=c("North Sea", "English Channel","Black Sea","Bay of Biscay",
                                                      "Mediterranean Sea","Subtropical ocean"))
  
# Plot
p1 <- ggplot(d1_agr, aes(fill=Location1, y=Value, x=Order)) + 
  ...
  theme(legend.title=element_text(face="bold")) +
  ylab("Number of CCs (%)") +
  labs(fill="Sea Regions")

plot(p1)
```

Occurence pattern for clusters shared between 2 sea regions (N=2)

```{r}
# Create data-frame
d2 <- data[data$N==2,]
d2_agr <- aggregate(d2$Order, list(d2$Location1, d2$Location2, d2$Order), FUN = length) # stats package
d2_agr2 <- data.frame(t(apply(d2_agr[,1:2],1,sort)))
d2_agr3 <- cbind(d2_agr2, d2_agr[,3:4])

names(d2_agr3)[1] <- "Location1"
names(d2_agr3)[2] <- "Location2"
names(d2_agr3)[3] <- "Order"
names(d2_agr3)[4] <- "Value"

d2_agr3$Shared_loc <- paste(d2_agr3$Location1, d2_agr3$Location2)
d2_agr4 <- aggregate(d2_agr3$Value, list(d2_agr3$Shared_loc, d2_agr3$Order), sum) # stats package
names(d2_agr4)[1] <- "Shared_loc"
names(d2_agr4)[2] <- "Order"
names(d2_agr4)[3] <- "Value"

# Calculate number of CCs shared between Mediterranean sea and Subtropical ocean
ms <- d2_agr4[d2_agr4$Shared_loc=="Mediterranean Sea Subtropical ocean",]
sum(ms$Value)

# Group rare pairs in "Others" (f<2.5%)
d2_agr4$Value[d2_agr4$Order=="Dino-Group-I"] <- ((d2_agr4$Value[d2_agr4$Order=="Dino-Group-I"])/nrow(df_I_2))*100
d2_agr4$Value[d2_agr4$Order=="Dino-Group-II"] <- ((d2_agr4$Value[d2_agr4$Order=="Dino-Group-II"])/nrow(df_II_2))*100
d2_agr4$Value[d2_agr4$Order=="Dino-Group-III"] <- ((d2_agr4$Value[d2_agr4$Order=="Dino-Group-III"])/nrow(df_III_2))*100
d2_agr4$Value[d2_agr4$Order=="Dino-Group-V"] <- ((d2_agr4$Value[d2_agr4$Order=="Dino-Group-V"])/nrow(df_V_2))*100
d2_agr4$Value[d2_agr4$Order=="Unknown"] <- ((d2_agr4$Value[d2_agr4$Order=="Unknown"])/nrow(df_uk_2))*100

d2_agr4$Shared_loc[d2_agr4$Value<3.5] <- "Others"

# Plot
p2 <- ggplot(d2_agr4, aes(fill=Shared_loc, y=Value, x=Order)) + 
...
meta_rda <- meta %>% select(1,9,11:22)
summary(meta_rda)
...
meta_rda <- CC_rda %>% select(1,((ncol(CC_rda))-9):ncol(CC_rda))
abund_rda <- CC_rda %>% select(1:(ncol(CC_rda)-13))
...
## Run & plot RDA (Figure 3A)

```{r}
# Redundancy analysis with formula interface
rda<-rda(formula = abund_rda ~ T + Chla + PO4 + NO3 + S + SIOH4 + O2, data = meta_rda)
...
In order to test the significativity of the RDA, an ANOVA was run on the global analysis and on each axis of the RDA. 
...
```{r}
## Test significativity of RDA
# anova.cca(rda, step=1000)

## Result
# Model: rda(formula = abund_rda ~ T + pH + Chla + PO4 + NO3 + S + SIOH4 + O2, data = meta_rda)
# Df Variance      F Pr(>F)    
# Model       7  0.08288 15.842  0.001 ***
# Residual 1049  0.78398                  
# ---
#  Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

# Test if each RDA axis is statisticaly significant --> bash script, run on slurm cluster 17.01.2022

# Bash script: Anova_RDA.sh
# Rscript: Rscript_Anova_RDA-copy.R
# Output: Rscript_Anova_RDA-copy.Rout
...
## Result
# Model: rda(formula = abund_rda ~ T + Chla + PO4 + NO3 + S + SIOH4 + O2, data = meta_rda)
# Df Variance       F Pr(>F)    
# RDA1        1  0.04405 58.9427  0.001 ***
#  RDA2        1  0.02254 30.1652  0.001 ***
#  RDA3        1  0.00686  9.1729  0.001 ***
#  RDA4        1  0.00324  4.3401  0.001 ***
#  RDA5        1  0.00265  3.5455  0.001 ***
#  RDA6        1  0.00246  3.2917  0.001 ***
#  RDA7        1  0.00107  1.4378  0.024 *  
#  Residual 1049  0.78398                   
# ---
#  Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

## Run completed in 1h07.
```
