...
# Get abundance of unknown metabarcodes  
# Get sum of unknown abundance per sample
s <- colSums(Kingdom[,11:158],na.rm = T)
# Get mean unknown abundance 
Kingdom_so<-mean(s)*100

#- Supergroup level -#
Supergroup <- subset(ASV_tax_N, Supergroup == "Unknown")
s <- colSums(Supergroup[,11:158],na.rm = T)
Supergroup_so<-mean(s)*100

#- Division level -#
Division <- subset(ASV_tax_N, Division == "Unknown")
s <- colSums(Division[,11:158],na.rm = T)
Division_so<-mean(s)*100

#- Class level -#
Class <- subset(ASV_tax_N, Class == "Unknown")
s <- colSums(Class[,11:158],na.rm = T)
Class_so<-mean(s)*100

#- Order level -#
Order <- subset(ASV_tax_N, Order == "Unknown")
s <- colSums(Order[,11:158],na.rm = T)
Order_so<-mean(s)*100

#- Family level -#
Family <- subset(ASV_tax_N, Family == "Unknown")
s <- colSums(Family[,11:158],na.rm = T)
Family_so<-mean(s)*100

#- Genus level -#
Genus <- subset(ASV_tax_N, Genus == "Unknown")
s <- colSums(Genus[,11:158],na.rm = T)
Genus_so<-mean(s)*100

#- Species level -#
Species <- subset(ASV_tax_N, Species == "Unknown")
s <- colSums(Species[,11:158],na.rm = T)
...
```{r}
d=read.csv("/shared/projects/formaldark/Silva/NA_silva_metadata_2_U.csv", h=T, sep=",",stringsAsFactors = T)
...
# Contribution of Dinoflagellata to global dataset
100*(nrow(d[d$Division=="Dinoflagellata",])/nrow(d))
...
# % Dinoflagellata within genus unassigned sequences
summary(don[don$Division =="Dinoflagellata",])
```
...
# Taxonomic assigment of Syndiniales species
table(synd$Species)

# % of Syndiniales metabarcodes having a species taxonomic assignment
100*nrow(synd[synd$Species!="Unknown",])/nrow(tax_all)
100*nrow(synd[synd$Species!="Unknown",])/nrow(synd)
```